#' Extrai valor com desconto negociado
#'
...
#' @export
get_valor_com_desconto_negociado <- function(resultado) {

  if (class(resultado) != 'character') {
    stop("\'resultado\' must be a character string")
  }

  str_match(resultado, 'com desconto negociado a \\d{1,2},?\\d{0,4}? % \\(valor com desconto: R.{1,4} ([0-9,\\.]+)\\)')[,2] %>%
    str_remove_all('\\s|\\.') %>%
